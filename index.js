...
const externalIp = '139.59.122.34';

app.listen(internalPort, '0.0.0.0', async () => {
    console.log(`Server Express Anda berjalan di port internal: ${internalPort}`);
    console.log(`Menggunakan IP manual: ${externalIp}`);
    console.log(`**Untuk mengakses aplikasi dari luar, gunakan IP publik server Anda (${externalIp}) dan port yang dialokasikan Pterodactyl (${internalPort}).**`);
    console.log(`Contoh URL akses: http://${externalIp}:${internalPort}`);
    console.log(`Pastikan Anda mendaftarkan URL webhook ini di Atlantic Pedia: http://${externalIp}:${internalPort}/webhook/atlantic`);
});